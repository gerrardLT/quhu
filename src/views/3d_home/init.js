...
// import foxModel from './resource/Fox.glb';
...
// init
export function initHome(Ammo,divs){
  ...
  function createPeople() {
    ...
    var marbleTexture = marble_loader.load(require('@/assets/jsm/earth.jpg'));
  ...
  function createBeachBall() {
    ...
    var beachTexture = texture_loader.load('@/assets/jsm/BeachBallColor.jpg');
  ...
  function loadFloydText() {
    ...
    text_loader.load(require('@/assets/jsm/Roboto_Regular.json'), function (font) {
      var xMid, text;
  ...
  function createTriangle(x, z) {
    ...
    // 将顶点数据设置为 BufferGeometry 的 position 属性
    ...
    // 将三角形的索引数据设置为 BufferGeometry 的 index 属性
  ...
  function startButtonEventListener() {
    for (let i = 0; i < divs.fadeOutDivs.length; i++) {
      divs.fadeOutDivs[i].classList.add('fade-out');
    }
    setTimeout(() => {
      document.getElementById('preload-overlay').style.display = 'none';
    }, 750);

    divs.startButton.removeEventListener('click', startButtonEventListener);
  ...
  manager.onLoad = function () {
    var readyStateCheckInterval = setInterval(function () {
      if (document.readyState === 'complete') {
        ...
        for (let i = 0; i < divs.preloadDivs.length; i++) {
          divs.preloadDivs[i].style.visibility = 'hidden'; // or
          divs.preloadDivs[i].style.display = 'none';
        }
        for (let i = 0; i < divs.postloadDivs.length; i++) {
          divs.postloadDivs[i].style.visibility = 'visible'; // or
          divs.postloadDivs[i].style.display = 'block';
        }
      }
    }, 1000);
  ...
  divs.startButton.addEventListener('click', startButtonEventListener);
